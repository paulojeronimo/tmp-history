...
:uri-my-tmp: {uri-my-domain}/tmp
:uri-my-github: https://github.com/paulojeronimo
:uri-tmp-github: {uri-my-github}/tmp
:uri-tmp-history-github: {uri-my-github}/tmp-history
// Attributes
:GPG: {uri-gpg}[GPG]
:GitHubPages: {uri-github-pages}[GitHub Pages]
:tmp: {uri-tmp-github}[tmp]
:tmp-history: {uri-tmp-history-github}[tmp-history]
// An alternative file where you can configure your variables pointed by this doc:
ifdef::my-variables[]
include::{my-variables}.adoc[]
endif::[]
...
files. This repo can also have Bash scripts and some other files that are
persistent in another git repository: {tmp-history}.

So, for the {tmp} repository, don't expect you can make a clone of it thinking
you'll pull it later because that won't work.
...
custom domain]. So, my `tmp` repo is available here: {uri-my-tmp}.

*The SHA256 of the temporary files* stored here keeps alive (and constantly
updated) in
{uri-tmp-history-github}/blob/main/changes.txt[tmp-history/blob/main/changes.txt].
...
[.big.text-center]
*Here is the list of the current scripts available for your use:*
...
persistent files that were changed to it and by generating the `changes.txt`
file.
...
.You want to remove the generated temporary files that have no content
[%collapsible]
====
----
...
to a `tmp-history` inside the parent directory of `tmp` repository.
====
